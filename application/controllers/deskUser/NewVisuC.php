...
class NewVisuC extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('deskUser/new_visu');
        $this->load->model('main');
    }

    function visu()
    {
        if (!$this->session->has_userdata('VISUFOLDERNEW'))
        {
            $this->session->set_userdata('VISUFOLDERNEW', mt_rand(1000000, 10000000));
        }
                        ...
                        else
                        {
                            if (file_exists($user_folder))
                            {
                                ...
                                $output = [
                                    ...
                                    'initialPreviewConfig' => [ [ 'url' => base_url() . "deskUser/newVisuC/uploadImgMain/delete"]],
                                    'append' => FALSE
                                ];
                                die(json_encode($output));
                            }
                        ...
                        else
                        {
                            if (file_exists($user_folder))
                            {
                                ...
                                $output = [
                                    ...
                                    'initialPreviewConfig' => [ [ 'url' => base_url() . 'deskUser/newVisuC/uploadImgAdditional/delete', 'key' => $filename],],
                                    'append' => TRUE
                                ];
                                die(json_encode($output));
                            }
    ...
    public function regist($type_visu)
    {
        ...
        $input_detail = array(
            ...
            'pingpong_table_' => $this->input->post('pingPongTable_'),
        ...
        $res_main = $this->new_visu->registMain($type_visu,$input_main);
        if ($res_main)
        {
            $res_detail = $this->new_visu->registDetail($res_main, $input_detail);
            if ($res_detail)
            {

                $this->session->unset_userdata('VISUFOLDERNEW');
